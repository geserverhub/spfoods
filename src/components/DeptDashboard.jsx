...
import { X, ChevronRight, Plus, FileText, Printer, BarChart2, Users, RefreshCw, Receipt, CreditCard, BookOpen, ScrollText } from 'lucide-react';
...
const pageContent = {
  ...
  report:      { th: 'รายงานบัญชี / งบดุล / กำไร-ขาดทุน', en: 'Accounting Report / Balance Sheet / P&L', ko: '회계 보고서 / 재무제표' },
...
const i18n = {
  th: { exit: 'ออก', soon: 'อยู่ระหว่างพัฒนา', soonSub: 'หน้านี้กำลังถูกสร้าง' },
  en: { exit: 'Exit', soon: 'Under Development', soonSub: 'This page is being built' },
  ko: { exit: '나가기', soon: '개발 중', soonSub: '이 페이지는 구축 중입니다' },
};
...
const ownerI18n = {
  th: { clickView: 'คลิกเพื่อดูข้อมูล' },
  en: { clickView: 'Click to view data' },
  ko: { clickView: '클릭하여 데이터 보기' },
};

export default function DeptDashboard({ dept, labels, token, ownerMenus, lang = 'th', setLang, onClose }) {
  const isOwner  = dept.id === 'service';
  const t        = i18n[lang] || i18n.th;
  const ot       = ownerI18n[lang] || ownerI18n.th;
              ...
              {ownerMenus.map((menu) => (
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">{ot.clickView}</p>
                  </div>
